use egui_geode::{egui, egui_glow, gui};
use gd::{geode::log, gl};
use std::{
...
pub extern "C" fn gui_context(reader: extern "C" fn(*const c_void)) {
    read_gui(|gui| reader(unsafe { transmute::<_, *const c_void>(gui.get_context()) }));
}

#[no_mangle]
pub extern "C" fn gui_add_run_fn(run_fn: extern "C" fn(*const c_void)) {
    mut_gui(|gui| gui.add_run_fn(unsafe { transmute::<_, fn(&egui::Context)>(run_fn) }));
}
...
pub extern "C" fn gui_send_mouse_pos(mouse_x: f32, mouse_y: f32) {
    mut_gui(|gui| gui.send_mouse_pos(egui::pos2(mouse_x, mouse_y)));
}
...
pub extern "C" fn gui_send_scroll_event(delta_x: f32, delta_y: f32) {
    mut_gui(|gui| gui.send_scroll_event(egui::vec2(delta_x, delta_y)));
}
...
#[no_mangle]
pub extern "C" fn gui_send_modifiers(
    shift: bool,
    ctrl: bool,
    alt: bool,
    mac_cmd: bool,
    ...
    mut_gui(|gui| {
        gui.send_modifiers(egui::Modifiers {
            alt,
            ctrl,
            shift,
            mac_cmd,
